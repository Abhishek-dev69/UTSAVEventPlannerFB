...
final class EditSubserviceViewController: UIViewController {
    ...
    private let saveButton = makePrimaryButton(title: "Save Changes")
    ...
    private func populateData() {
        guard let sub = subserviceToEdit else { return }
        nameField.text = sub.name
        rateField.text = "\(sub.rate)"
        isFixedRate = sub.isFixed
    }
...
fileprivate func makePrimaryButton(title: String) -> UIButton {
    let purple = UIColor(red: 138/255, green: 73/255, blue: 246/255, alpha: 1)
    let b = UIButton(type: .system)
    b.setTitle(title, for: .normal)
